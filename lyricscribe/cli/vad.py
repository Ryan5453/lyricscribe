...
class VAD:
    ...
    def __init__(
        self,
        threshold: float = 0.5,
        min_speech_theshold_ms: int = 250,
        min_silence_threshold_ms: int = 250,
    ):
    ...
    def process_file(self, input_path: Path, output_path: Path) -> bool:
        ...
        try:
            ...
            timestamps = get_speech_timestamps(
                ...
                min_speech_duration_ms=self.min_speech_threshold_ms,
                min_silence_at_max_speech=self.min_silence_threshold_ms,
                return_seconds=True,
            )
            ...
            logger.info(
                f"{input_path.name}: {output['speech_percentage']}% speech, {len(timestamps)} segments"
            )
            return True
    ...
    def process_directory(self, directory: Path, filename: str) -> str:
        ...
        for subdir in subdirs:
            ...
            logger.info(
                f"Ran VAD on {input_path}, {success} successes, {fail} failues, {skipped} skips"
            )
